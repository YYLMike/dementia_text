...
def segmentation(sentence, segment_tool, segmenter=None):
...
class Prior_feature_extract:
    ...
    def pos_tag_analysis(self, sentence, segment_tool):  # segment_tool, 0:jieba, 1: Ckip
        ...
        word_pos = segmentation(sentence, segment_tool, self.segmenter)
    ...
    def lexical_rich_analysis(self, sentence, segment_tool):
        ...
        if segment_tool == 'jieba':
            self.semantic_features_jieab.append([ttr])
        elif segment_tool == 'ckip':
            self.semantic_features_ckip.append([ttr])

        return ttr

    def main():
        test_prior = Prior_feature_extract()

        test_sentence = '這個孩子拿了個東西放到上面，有一個流水台，這是他的家庭主婦，這是他的兒子。'
        postag_jieba = test_prior.pos_tag_analysis(
            test_sentence, 'jieba')
        print('sentence postag ratio (n,v,a,r) by jieba: ', postag_jieba)
        postag_ckip = test_prior.pos_tag_analysis(
            test_sentence, 'ckip')
        print('sentnece postag ratio (n,v,a,r) by ckip: ', postag_ckip)
        ttr_jieba = test_prior.lexical_rich_analysis(test_sentence, 'jieba')
        print('ttr jieba: ', ttr_jieba)
        ttr_ckip = test_prior.lexical_rich_analysis(test_sentence, 'ckip')
        print('ttr_ckip: ', ttr_ckip)
        ...
        # print(postag_ckip)
        # print('*'*100)
        print('synatactic_f jieab: ',
              test_prior.syntactic_features_jieba)
        print('syntactic_f ckip: ', test_prior.syntactic_features_ckip)
        print('semantic_f jieba: ', test_prior.semantic_features_jieab)
        print('semantic_ckip: ', test_prior.semantic_features_ckip)

if __name__ == '__main__':
    Prior_feature_extract.main()
