...
x_onehot_w2v, vocab_processor = data_preprocess.text_to_onehot_w2v(x_seg, w2v_model.wv.vocab.keys())

#Split data into train and validate part
dev_sample_index = -1 * int(.1*float(len(y)))
x_train, x_dev = x_onehot_w2v[:dev_sample_index], x_onehot_w2v[dev_sample_index:]
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        cnn = TextCNN_w2v_pretrain.TextCNN_w2v_pretrain(sequence_length=SEQUENCE_LENGTH, 
                     num_classes=2, 
        ...
        grads_and_vars = optimizer.compute_gradients(cnn.loss) # first part of minimize()
        train_op = optimizer.apply_gradients(grads_and_vars, global_step=global_step) # second part of minimize()
